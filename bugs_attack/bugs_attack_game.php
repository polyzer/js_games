<?php 
	session_start();
	$_SESSION["bugs_attack"]["true_connection"] = "true";
?>
...
<head>
	<meta charset="UTF-8" />
	<title>Pixel Bugs Attack</title>
	<script src="https://cdn.rawgit.com/konvajs/konva/0.11.1/konva.min.js"></script>
	<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
	<script src="//vk.com/js/api/xd_connection.js?2"  type="text/javascript"></script>

	<link rel="stylesheet" type="text/css" href="./bugs_attack_game_style.css" />
</head>

<body>	
<img width = "30" height = "52" id="bug_1_red_stop_img" class="hidden_imgs" src="../games_resources/bugs_attack/images/bug_1_red_stop.png" />
<img width = "30" height = "52" id="bug_1_red_walk_img" class="hidden_imgs" src="../games_resources/bugs_attack/images/bug_1_red_walk.png" />
<img width = "30" height = "52" id="bug_1_red_dead_img" class="hidden_imgs" src="../games_resources/bugs_attack/images/bug_1_red_dead.png" />
<img width = "40" height = "35" id="FloorHole_img" class="hidden_imgs" src="../games_resources/bugs_attack/images/floor_hole.png" />
<img width = "40" height = "35" id="FloorHoleRepaired_img" class="hidden_imgs" src="../games_resources/bugs_attack/images/floor_hole_repaired.png" />
<img width = "20" height = "30" id="Hammer_img" class="hidden_imgs" src="../games_resources/bugs_attack/images/hammer.png" />
<img width = "20" height = "15" id="Cheese_img" class="hidden_imgs" src="../games_resources/bugs_attack/images/cheese.png" />	
<img width = "15" height = "10" id="Crumbs_img" class="hidden_imgs" src="../games_resources/bugs_attack/images/crumbs.png" />	
...
<div id="GameRating">
	<table id="GameRatingTable">
		<tr>
			...
			<td>Аватар</td>
			<td>Имя</td>
			<td><img style="align: center;" width="40" height="57" src="../games_resources/bugs_attack/images/bug_1_red_dead.png" /></td>
			<td>Время</td>
		</tr>
		<tr>
			<td>1</td>
...
<div id="GameResult">
	...
	<div id="ResultBlock">
		<div id="EnemiesKilledResult">
			<div id="EnemiesKilledResultText" class="Text">
			</div>
		</div>
		<div id="TimeResult">
	...
	if (ServerAnswerDatas.server_answer == "HAVE_RATING")
	{
		...
		VK.api("users.get", {"user_ids": ids_str, "fields": "photo_50"}, function (data) {
			...
			for (var i = 0; i < data.response.length; i++)
			{
				...
				GetsObj.children[0].children[i+1].children[3].innerHTML = ServerAnswerDatas.result_datas.best_rating[i].enemies_killed_max;
...
function getResultsRequest(json_params)
{
	...
	xmlhttp1.open('POST', './bugs_attack_funcs.php', false);
...
function saveResultsRequest(json_params)
{
	...
	SendDatas = {
		...
		EnemiesKilled: json_params.gamestats.EnemiesKilledCounter,
...
function doRequest (onFunction, SendDatas) 
{
	...
	xmlhttp.open('POST', './bugs_attack_game_funcs.php', true);
...
function SurvivalModeParameters () {
	...
	this.KilledEnemiesCountForCreateNewFood = 10;
	...
	this.InitDatasEnemiesHealthStep = 25;
	this.InitDatasEnemiesHealthStepTime = 30;
...
function LevelModeParameters () {
	...
	this.InitDatasFloorHoleHealthStep = 50;
	this.InitDatasFloorHoleHealthStepTime = 30;
	this.InitDatasEnemiesHealthStep = 30;
	this.InitDatasEnemiesHealthStepTime = 30;
	this.InitDatasFoodHealthStep = 30;
	this.InitDatasFoodHealthStepTime = 30;
...
function _GameStats () { // статистика!
		...
		this.EnemiesKilledDiv = document.createElement("div");		
		this.EnemiesKilledDiv.style.height = "100%";
		this.EnemiesKilledDiv.style.width = "25%";
		this.EnemiesKilledDiv.style.float = "left";
		this.EnemiesKilledDiv.style.backgroundColor = "#f5f1cb";
		this.Div.appendChild(this.EnemiesKilledDiv);
		...
		this.EnemiesKilledCounter = 0; // счетчик убитых крыс
		// этот параметр должен контролировать создание только 1 сыра!
		this.LastFoodAddEnemiesKilledCount = 0;
...
{
	this.EnemiesKilledDiv.innerHTML = "Крыс убито: " + this.EnemiesKilledCounter;
...
_GameStats.prototype.increaseEnemiesKilledCounter = function () 
{
	this.EnemiesKilledCounter++;
	this.updateDivs();
}

_GameStats.prototype.reduceEnemiesKilledCounter = function () 
{
	this.EnemiesKilledCounter--;
	this.updateDivs();
}
...
// принимает параметры:
// 
function _Enemy (json_params) 
{
		this.Members = {};
		
		this.Members.ImgObjs = {};
		this.Members.ImgObjs.Default = document.getElementById("bug_1_red_stop_img");
		this.Members.ImgObjs.Attack = document.getElementById("bug_1_red_walk_img");
		this.Members.ImgObjs.Dead = document.getElementById("bug_1_red_dead_img");
		this.Members.ImgObjs.Damage = document.getElementById("bug_1_red_damage_img");
...
//		this.Members.AttackDistance = null; // дистанция атаки
		...
		this.Image().EnemyObj = this;
		/// возможно здесь косяк!!!
		this.Image().on('click', function (event) {
			event.target.EnemyObj.onClick({"Weapon" : Weapon});
		});
		...
		this.Layer().draw();
		console.log("_Enemy: Я родился");

}	
////////////////////////////////////////////////////////////
// get/set members functions!!!!!!!!!!!!!!////////////////

_Enemy.prototype.isDead = function ()
{
	if (this.Status() == "Dead")
	{
		return 1;
...
_Enemy.prototype.isGoing = function ()
{
...
_Enemy.prototype.Layer = function(Value)
{
	if (Value !== undefined)
	{
...
_Enemy.prototype.Speed = function(Speed)
{
	if (Speed !== undefined)
	{
...
_Enemy.prototype.SpeedLimit = function(SpeedLimit)
{
...
_Enemy.prototype.SpeedFactor = function(Value)
{
	if (Value !== undefined)
	{
...
_Enemy.prototype.Step = function(Value)
{
	if (Value !== undefined)
	{
...
_Enemy.prototype.Health = function (Health)
{
	if (Health !== undefined)
	{
...
_Enemy.prototype.Height = function (Height)
{
	if (Height !== undefined)
	{
...
_Enemy.prototype.Width = function (Width)
{
	if (Width !== undefined)
	{
...
_Enemy.prototype.Damage = function (Value)
{
	if (Value !== undefined)
	{
...
_Enemy.prototype.DamageFactor = function (Value)
{
	if (Value !== undefined)
	{
...
_Enemy.prototype.AttackDistance = function (Value)
{
	if (Value !== undefined)
	{
...
_Enemy.prototype.ImgObjs = function (Value) 
{
	if (Value !== undefined)
	{
...
_Enemy.prototype.Image = function (Value) 
{
	if (Value !== undefined)
	{
...
_Enemy.prototype.Status = function (Value)
{
	if (Value !== undefined)
	{
...
_Enemy.prototype.Target = function (Value)
{
	if (Value !== undefined)
	{
...
_Enemy.prototype.init = function (json_params)
{
...
_Enemy.prototype.Life = function (json_params)
{
...
/*
// на дистанции атаки!
_Enemy.prototype.isAtAttackDistance = function (json_params)
{
	if (this.Target() != null)
	{
	...
	{
			console.log("from _Enemy.isAtAttackDistance: Нет цели!");
			return 0;
	}		
}
*/
...
_Enemy.prototype.selectNearestTarget = function (json_params)
{	
	if (json_params !== undefined) // если есть входные параметры
	{
		...
		console.log(this.constructor.name + ": selecting Target from " + json_params.Targets.length);
			if (json_params.Targets.length == 0){ // если в массиве нет
				...
				console.log("from _Enemy.selectNearestTarget: Targets array is empty!!!!");
				return;
			}
...
_Enemy.prototype.selectRandomTarget = function (json_params)
{	
	if (json_params !== undefined) // если есть входные параметры
	{
		if (json_params.Targets !== undefined) //если есть массив с целями
		...
		console.log(this.constructor.name + ": selecting Target from " + json_params.Targets.length);
			if (json_params.Targets.length == 0){ // если в массиве нет
				...
				console.log("from _Enemy.selectRandomTarget: Targets array is empty!!!!");
				return;
			}
...
_Enemy.prototype.select1of5Target = function (json_params)
{	
	if (json_params !== undefined) // если есть входные параметры
	{
		if (json_params.Targets !== undefined) //если есть массив с целями
		...
		console.log(this.constructor.name + ": selecting Target from " + json_params.Targets.length);
			if (json_params.Targets.length == 0){ // если в массиве нет
				...
				console.log("from _Enemy.select1of3Target: Targets array is empty!!!!");
				return;
			}
...
_Enemy.prototype.calculateAttackPoint = function (json_params)
{
	...
	{
		console.log("from _Enemy.calculateAttackPoint: Нет цели!");
	}
}

_Enemy.prototype.isCanAttack = function ()
{
...
/*
_Enemy.prototype.calculateAttackDistance = function (json_params)
{
	if (this.Target() != null)
	{
	...
	{
		console.log("from _Enemy.calculateAttackDistance: Нет цели!");
	}
}
*/


_Enemy.prototype.increaseSpeed = function (json_params) 
{
	if(json_params !== undefined)
	{
...
// понижение скорости
_Enemy.prototype.reduceSpeed = function (json_params) 
{
	if(json_params !== undefined)
	{
...
// когда крыса убита
_Enemy.prototype.onKill = function (json_params) 
{
...
_Enemy.prototype.reduceHealth = function (json_params)
{
	if (json_params !== undefined)
	{
...
_Enemy.prototype.increaseHealth = function (json_params)
{
	if(json_params)
	{
...
_Enemy.prototype.onAttackMe = function (json_params) 
{
	if (json_params !== undefined)
	{
		if (json_params.Damage)
...
//атака цели
_Enemy.prototype.attackTarget = function (json_params)
{
	if (json_params.Target !== undefined)
	{
...
//поворот к цели!
// данная функция вызывается в _Enemy.comeTo
_Enemy.prototype.turnToTarget = function (json_params) 
{
	this.Image().rotation(1/2 * 180);
	if (json_params.Target !== undefined)
	{
...
/*
_Enemy.prototype.startAttackAnim = function (json_params) 
{
	this.Image().image(this.ImgObjs().Attack);
}

_Enemy.prototype.stopAttackAnim = function (json_params) 
{
	this.Image().image(this.ImgObjs().Default);	
}

_Enemy.prototype.startDeadAnim = function (json_params) 
{
	this.Image().image((this.ImgObjs().Default));
}

_Enemy.prototype.stopDeadAnim = function (json_params) 
{
	this.Image().image((this.ImgObjs().Dead));	
}
*/
_Enemy.prototype.onClick = function (json_params)
{
	if(json_params !== undefined)
	{
...
_Enemy.prototype.comeTo = function (json_params)
{
		if (json_params)
		{
			if (json_params.Target)
...
/*
_Enemy.prototype.comeTo2 = function (json_params)
{
...
//	document.body.style.cursor = 'url("../games_resources/bugs_attack/images/hammer.png"), pointer';
...
function _FloorHole (json_params) 
{
	...
	this.Members.Health = null;
	this.Members.Timers = {};
	this.Members.Timers.enemyCreationTime = null;
	
	this.Members.Enemies = null;
...
_FloorHole.prototype.init = function (json_params)
{
	...
	{	
		...
		if (json_params.Enemies !== undefined)
		{
			this.Enemies = json_params.Enemies; // массив
		}
...
_FloorHole.prototype.createEnemy = function (json_params)
{
	
// могут возникнуть проблемы!	
	var FloorThat = this;
	this.enemyCreationTimer = setTimeout(function () 
		{
			InitDatas._Enemy.X = FloorThat.X() + (Math.random() * 10 -3);
			InitDatas._Enemy.Y = FloorThat.Y() + (Math.random() * 10 -3);
			// добавление крысы 
			FloorThat.Enemies.push(new _Enemy(InitDatas._Enemy));
			// возвращаем статус на открыта!
			FloorThat.Status("Open");
		},
		// здесь параметры
		(Math.random() * json_params.InitDatas._FloorHole.createEnemyTimeTo + json_params.InitDatas._FloorHole.createEnemyTimeFrom) * 1000);
	// устанавливаем статус создание крысы, чтобы нас не удалили из
	// массива!	
	this.Status("EnemyCreating");
}
...
{
	clearTimeout(this.enemyCreationTimer);
...
var DefaultInitDatas = {
	_Enemy : {
		Health: 170,
		ImgObjs: {
			Default: document.getElementById("bug_1_red_stop_img"),
			Dead: document.getElementById("bug_1_red_dead_img"),
			Damage: document.getElementById("bug_1_red_damage_img"),
			Attack: document.getElementById("bug_1_red_attack_img")
		},
		Speed: 30,
		SpeedLimit: 100,
		SpeedFactor: 1,
...
// json_params.MyVK
		...
		$("#EnemiesKilledResultText").html("Крыс убито: " + json_params.Stats.EnemiesKilledCounter);
...
function GameProcess (json_params)
{
	...
	{
		if (gamestats.Timer % json_params.CurrentGameParams.InitDatasEnemyHealthStepTime  == 0)
		{
			InitDatas._Enemy.Health += json_params.CurrentGameParams.InitDatasEnemyHealthStep;
		}
		...
		if((gamestats.EnemiesKilledCounter != 0) && 
			 ((gamestats.EnemiesKilledCounter % json_params.CurrentGameParams.KilledEnemiesCountForCreateNewFood) == 0)&&
			  gamestats.LastFoodAddEnemiesKilledCount != gamestats.EnemiesKilledCounter)
		{
	...
	for(var i = 0; i < Enemies.length; i++)
	{
		...
		{
			Enemies.splice(i, 1);
			gamestats.increaseEnemiesKilledCounter();
		} else
		{
			Enemies[i].Life({"Targets" : Foods});
		}
	}
...
function GameInit(json_params)
{
	...
	InitDatas._Enemy.Layer = MainLayer;
	InitDatas._FloorHole.Layer = MainLayer;
	InitDatas._Food.Layer = MainLayer;
	InitDatas._FloorHole.Enemies = Enemies;
