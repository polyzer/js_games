<?php 
	session_start();
if ($_SESSION["bugs_attack"]["true_connection"] && 
	$_POST["Datas"]
   ) 
{
...
////////////////// проверка!!!!!!!!!!!!!!!!!!!
			...
			if ($datas["EnemiesKilled"])
			{
				$query = "UPDATE `results` SET `enemies_killed_max` = '". ((int)$datas["EnemiesKilled"] + (int)$result_arr["result_datas"]["user_results"]["enemies_killed_max"])."' WHERE `vk_id`='".$datas["vk_id"]."';";
	...
	if ($datas["Operation"] === "get_rating_by_num") {
		$res = $mysqli->query("SELECT * FROM `results` ORDER BY `enemies_killed_max` DESC LIMIT ".$datas["RateNum"]." ;");
		if ($res->num_rows != 0){
			$i = 0;
			while($row = $res->fetch_assoc())
			{
					...
					$result_arr["result_datas"]["best_rating"][$i]["enemies_killed_max"] = $row["enemies_killed_max"];
